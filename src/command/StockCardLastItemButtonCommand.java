...
public class StockCardLastItemButtonCommand implements Command {

	private StockCardView cardView;
	private StockCardRepository cardRepository = new StockCardRepository();

	public StockCardLastItemButtonCommand(StockCardView stockCardView) {
		super();
	...
	public void execute() {
		try (Session session = HibernateUtil.getSessionFactory().openSession()) {
			...
			Query query = session.createNativeQuery(Constant.GET_LAST_ITEM_FROM_STOCK_CARD_TABLE);
