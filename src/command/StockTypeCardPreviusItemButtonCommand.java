...
public class StockTypeCardPreviusItemButtonCommand implements Command {
	private StockTypeCardCreateUi cardCreateUi;
	private StockTypeCardRepository cardRepository;

	public StockTypeCardPreviusItemButtonCommand(StockTypeCardCreateUi cardCreateUi) {
	...
	public void execute() {
		...
		try (Session session = HibernateUtil.getSessionFactory().openSession()) {
			...
			String sql = "select * from stocktypetbl  where StockTypeId = (select Max(StockTypeId) from stocktypetbl where StockTypeId <"
